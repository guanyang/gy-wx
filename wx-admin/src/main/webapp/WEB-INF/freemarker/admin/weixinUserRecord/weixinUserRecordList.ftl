...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>微信用户记录报表</title>
  </head>
  <body> 
  	<#include "/WEB-INF/freemarker/admin/wxTypeHidden.ftl">
	<div id="searchPanel" class="easyui-panel" title="微信用户记录报表"
			...
			<form id="searchFrm">
				<table class="tblContent" style="width:100%;">
							...
							<tr>
								...
								<td class="tdLeft"><span>关注状态:</span></td>
								<td class="tdRight">
		...
		<div id="tbMenus" style="padding:2px;height:auto">
			...
			<a href="javascript:void(0)" class="easyui-linkbutton" plain="true" iconCls="icon-reload" onclick="javascript:updateUserInfo();">同步用户信息 </a>
		</div>
				<!--添加窗口-->
		<div id="addDialog" class="easyui-dialog" iconCls="icon-add"
			style="width: 40%; height: 40%;" closed="true" modal="true"
			...
			<form id="addForm" method="post">
				<table class="formbox" border="0" cellpadding="5px" cellspacing="5px" style="width:100%">
							<input type="hidden" id="idAdd" name="id" />
								...
								<tr>
									<td style="text-align:right;"><span style="color:red">*</span>关注状态：</td>
									<td>
									<select class="easyui-combobox" required="true" id="subStatusAdd" name="subStatus" style="width:100px;" panelHeight="80px" editable="false">
									    <option value="1">已关注</option>
									    <option value="2">取消关注</option>
									</select>
									</td>
	...
	<!--引入js路径-->
	<@resMacro.jsurl resRoot="${javascriptServerAdmin}" url=["/js/weixinUserRecord/weixinUserRecordList"] resConcat="${resConcat}" />
  </body>
</html>
